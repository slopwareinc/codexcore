...
public final class MessageParser: Sendable {
    ...
    private let priorityRegex = try! NSRegularExpression(
        pattern: "(?i)^\\[(p[0-3])\\]\\s*",
        options: []
    )
    ...
    public func parseCodeReview(text: String) -> ConversationCodeReviewData? {
        ...
        if trimmed.hasPrefix("{") && trimmed.hasSuffix("}") {
            if let parsed = parseCodeReviewFromJSONStr(trimmed) {
                return parsed
            }
        }

        for candidate in extractFencedJSONCandidates(trimmed) {
            if let parsed = parseCodeReviewFromJSONStr(candidate) {
                return parsed
            }
        }
    ...
    private func extractFencedJSONCandidates(_ text: String) -> [String] {
        let lines = text.components(separatedBy: "\n")
        ...
        while index < lines.count {
            ...
            let language = String(trimmed.dropFirst(opening.length)).trimmingCharacters(in: .whitespacesAndNewlines).lowercased()
    ...
    private func extractJSONObjectCandidates(_ text: String) -> [String] {
        let bytes = Array(text.utf8)
        var startIndex = 0
        var candidates: [String] = []
    ...
    private struct RawCodeReviewFinding: Decodable {
        let title: String
        let body: String
        let confidence_score: Double
        let priority: Int?
        let code_location: RawCodeReviewCodeLocation?
    }

    private struct RawCodeReviewCodeLocation: Decodable {
        let absolute_file_path: String
        let line_range: RawCodeReviewLineRange?
    }

    private struct RawCodeReviewLineRange: Decodable {
        let start: Int
        let end: Int
    }

    private struct RawCodeReviewPayload: Decodable {
        let findings: [RawCodeReviewFinding]
        let overall_correctness: String?
        let overall_explanation: String?
        let overall_confidence_score: Double?
    }

    private func parseCodeReviewFromJSONStr(_ text: String) -> ConversationCodeReviewData? {
        guard let data = text.data(using: .utf8) else { return nil }

        let decoder = JSONDecoder()
        guard let raw = try? decoder.decode(RawCodeReviewPayload.self, from: data) else {
            return nil
        }
        ...
        for finding in raw.findings {
            ...
            guard !trimmedTitle.isEmpty && !trimmedBody.isEmpty else { continue }

            let range = NSRange(location: 0, length: trimmedTitle.utf16.count)
            var inferredPriority: Int? = nil
            if let match = priorityRegex.firstMatch(in: trimmedTitle, options: [], range: range) {
                let pRange = match.range(at: 1)
                if let pSubstring = Range(pRange, in: trimmedTitle) {
                    let pVal = trimmedTitle[pSubstring].lowercased().replacingOccurrences(of: "p", with: "")
                    inferredPriority = Int(pVal)
                }
            }
            let normalizedTitle = priorityRegex.stringByReplacingMatches(in: trimmedTitle, options: [], range: range, withTemplate: "").trimmingCharacters(in: .whitespacesAndNewlines)

            let priority = finding.priority ?? inferredPriority

            var location: ConversationCodeReviewLocation? = nil
            if let loc = finding.code_location {
                let absolutePath = normalizeReviewPath(loc.absolute_file_path)
                var range: ConversationCodeReviewLineRange? = nil
                if let r = loc.line_range {
                    range = ConversationCodeReviewLineRange(start: r.start, end: max(r.start, r.end))
                }
                location = ConversationCodeReviewLocation(absoluteFilePath: absolutePath, lineRange: range)
            }

            findings.append(ConversationCodeReviewFinding(
    ...
    private func normalizeReviewPath(_ path: String) -> String {
        ...
        for prefix in ["//wsl$/", "//wsl.localhost/"] {
            if lower.hasPrefix(prefix) {
                ...
                if let firstSlash = remainder.firstIndex(of: "/") {
                    let pathAfterDistro = remainder[firstSlash...]
                    return collapseRepeatedSlashes(String(pathAfterDistro))
                }
                return "/"
            }
        }
        ...
        if let match = driveRegex.firstMatch(in: normalized, options: [], range: range) {
            if let dRange = Range(match.range(at: 1), in: normalized),
        ...
        if normalized.hasPrefix("//") {
            let remainder = collapseRepeatedSlashes(String(normalized.dropFirst(2)))
            return "//\(remainder)"
        }
    ...
    private func collapseRepeatedSlashes(_ path: String) -> String {
        var collapsed = ""
