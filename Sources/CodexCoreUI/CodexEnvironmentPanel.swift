...
public struct CodexProjectEnvironmentPanel: View {
    ...
    private func select(_ selection: CodexProjectEnvironmentSelection) {
        ...
        case .worktree:
            prepareHandoff()
        case .cloud:
            // The app-server/cloud provider is intentionally not wired in this
            // local parity slice. Keep the selected location visible while
            // refusing to imply that a remote run was started.
            session.environment.selection = .cloud
            session.lastActivity = CodexActivity(
                kind: .notice,
                title: "Cloud environment unavailable",
                detail: "Cloud runs are not configured on this local host."
            )
            session.resultCard = nil
        }
    }
