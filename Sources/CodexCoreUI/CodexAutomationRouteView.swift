...
private struct CodexAutomationRow: View {
        ...
        HStack(spacing: 14) {
            ...
            VStack(alignment: .leading, spacing: 5) {
                ...
                Text(automation.schedule.summary)
                    .font(theme.fonts.caption)
                    .foregroundStyle(theme.colors.textSecondary)
                if let previousRun = automation.previousRun {
                    Text(previousRunSummary(previousRun))
                        .font(theme.fonts.caption)
                        .foregroundStyle(theme.colors.textSecondary)
                }
    ...
    private func previousRunSummary(_ run: CodexAutomationRunRecord) -> String {
        let formatter = DateFormatter()
        formatter.dateStyle = .short
        formatter.timeStyle = .short
        let date = formatter.string(from: run.finishedAt ?? run.startedAt)
        switch run.status {
        case .running: return "Last run started \(date)"
        case .succeeded: return "Last run finished \(date)"
        case .failed, .cancelled: return "Last run failed \(date)"
        }
    }
