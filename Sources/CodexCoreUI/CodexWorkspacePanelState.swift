...
///
/// This owns the tool *sessions* — terminals (ghostty) and browsers (WebKit) —
/// along with the panel's open/selection/width state. It is deliberately a
/// reference type held outside any transient SwiftUI view so that hiding the
/// panel or switching chats only recycles the view *surface*; the underlying
/// PTYs and web views stay alive until the session is explicitly closed or the
/// owning store evicts this chat.
@MainActor
public final class CodexWorkspacePanelState: ObservableObject {
    ...
    public var hasOpenTools: Bool {
        !terminalSessions.isEmpty || !browserSessions.isEmpty
    }
    ...
    private func firstAvailableTabID(_ fallbackTabIDs: [String]) -> String? {
        terminalSessions.first?.id ?? browserSessions.first?.id ?? fallbackTabIDs.first
    }
}
