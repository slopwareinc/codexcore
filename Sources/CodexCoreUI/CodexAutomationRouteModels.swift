...
public enum CodexAutomationRouteAction: Equatable, Sendable {
    ...
    /// Clears local unread state for chats produced by these automations.
    case markAllRead
    /// Pauses all local definitions; existing run chats remain untouched.
    case archiveAll
    /// Deletes all local definitions; existing run chats remain untouched.
    case deleteAll
    case learnMore
    ...
    public var title: String {
        ...
        case .markAllRead: "Mark all runs read"
        case .archiveAll: "Pause all automations"
        case .deleteAll: "Delete all automations"
    ...
    public var draftRequest: CodexAutomationDraftRequest? {
        ...
        case .save, .toggle, .delete, .runNow, .markAllRead, .archiveAll, .deleteAll, .learnMore:
            nil
        }
    }
...
/// A local record of one scheduled run. Run history is intentionally kept
/// outside the official automation TOML schema; it is client metadata attached
/// to the ordinary Codex thread created for the run.
public struct CodexAutomationRunRecord: Identifiable, Codable, Equatable, Sendable {
    ...
    public var isFinished: Bool { finishedAt != nil && status != .running }
}
...
public struct CodexAutomation: Identifiable, Codable, Equatable, Sendable {
    ...
    /// Bounded local history for the most recent runs, oldest first. This is
    /// persisted in a sidecar by `CodexAutomationFileStore`, never in the
    /// official automation TOML document.
    public var previousRuns: [CodexAutomationRunRecord]
    ...
    public init(
        ...
        previousRuns: [CodexAutomationRunRecord] = []
    ) {
        ...
        self.previousRuns = Array(previousRuns.suffix(CodexAutomation.maxPreviousRunCount))
    }

    public static let maxPreviousRunCount = 20

    public var statusLabel: String { status.label }
    public var isEnabled: Bool { status != .disabled }
    public var previousRun: CodexAutomationRunRecord? { previousRuns.last }
}
...
public struct CodexAutomationRouteSession: Equatable, Sendable {
    ...
    public mutating func perform(_ action: CodexAutomationRouteAction) -> CodexAutomationDraftRequest? {
        ...
        case .archiveAll:
            state.automations = state.automations.map { automation in
                var paused = automation
                paused.status = .disabled
                paused.nextRunAt = nil
                return paused
            }
        case .deleteAll:
            state.automations.removeAll()
        default:
            break
        }
...
public struct CodexAutomationLifecycle: Equatable, Sendable {
    ...
    public static let maxPreviousRunCount = CodexAutomation.maxPreviousRunCount
    ...
    public mutating func save(_ automation: CodexAutomation, now: Date = Date()) {
        ...
        automation.previousRuns = Array(automation.previousRuns.suffix(Self.maxPreviousRunCount))
    ...
    /// Pauses every local definition. This is the local equivalent of an
    /// archive-all action; it does not claim that app-server supports a bulk
    /// automation RPC and leaves prior run chats untouched.
    @discardableResult
    public mutating func archiveAll() -> [CodexAutomation] {
        var changed: [CodexAutomation] = []
        for index in automations.indices where automations[index].status != .disabled {
            automations[index].status = .disabled
            automations[index].nextRunAt = nil
            changed.append(automations[index])
        }
        return changed
    }

    @discardableResult
    public mutating func deleteAll() -> [String] {
        let ids = automations.map(\.id)
    ...
    public mutating func beginRun(id: String, now: Date = Date()) -> CodexAutomation? {
        ...
        automations[index].nextRunAt = automations[index].schedule.nextDate(after: now)
        automations[index].previousRuns.append(CodexAutomationRunRecord(startedAt: now))
        trimHistory(at: index)
        return automations[index]
    }
    ...
    public mutating func finishRun(id: String, threadID: String?, error: String?, now: Date = Date()) {
        ...
        if let recordIndex = automations[index].previousRuns.lastIndex(where: { $0.status == .running }) {
            automations[index].previousRuns[recordIndex].status = error == nil ? .succeeded : .failed
            automations[index].previousRuns[recordIndex].finishedAt = now
            automations[index].previousRuns[recordIndex].threadID = threadID
            automations[index].previousRuns[recordIndex].error = error
        } else {
            automations[index].previousRuns.append(CodexAutomationRunRecord(
    ...
    private mutating func trimHistory(at index: Int) {
        guard automations.indices.contains(index), automations[index].previousRuns.count > Self.maxPreviousRunCount else { return }
        automations[index].previousRuns.removeFirst(automations[index].previousRuns.count - Self.maxPreviousRunCount)
    }
