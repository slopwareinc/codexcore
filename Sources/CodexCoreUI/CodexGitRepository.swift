...
public actor CodexGitRepository {
    ...
    /// Moves the current local changes into a newly-created Git worktree.
    ///
    /// This is deliberately kept behind the serialized repository actor. A
    /// named stash is retained until the target worktree has been verified, so
    /// a failed apply leaves a recovery object instead of silently losing
    /// staged, tracked, or untracked changes.
    public func handOffToWorktree(_ request: CodexWorktreeHandoffRequest) async throws -> CodexWorktreeHandoffResult {
        ...
        guard source.path == root.path else {
        ...
        guard target.path != root.path, !target.path.hasPrefix(root.path + "/") else {
            throw CodexGitRepositoryError.commandFailed(
                ...
                message: "The worktree destination must be outside the source repository."
            )
        }
        ...
        guard try await optional(["check-ref-format", "--branch", branch]) != nil,
              try await optional(["show-ref", "--verify", "--quiet", "refs/heads/\(branch)"]) == nil else {
            throw CodexGitRepositoryError.commandFailed(
                command: "git worktree add",
                message: "Invalid or already-existing branch name."
            )
        }

        let snapshot = try await snapshot(source: .uncommitted)
        let hasChanges = snapshot.hasUncommittedChanges
        var stashOID: String?
        if hasChanges {
            _ = try await run([
                "stash", "push", "--include-untracked", "--message",
                "Codex worktree handoff: \(branch)",
            ])
            stashOID = try await run(["rev-parse", "refs/stash"])
                .stdout.trimmingCharacters(in: .whitespacesAndNewlines)
            guard let oid = stashOID, !oid.isEmpty else {
                throw CodexGitRepositoryError.partialSuccess(
                    "Changes were stashed but the recovery object could not be identified. Inspect `git stash list` before continuing."
                )
            }
        }

        do {
            _ = try await run(["worktree", "add", "-b", branch, target.path, "HEAD"])
        } catch {
            if let stashOID {
                do {
                    _ = try await run(["stash", "apply", "--index", stashOID])
                    _ = try await dropHandoffStash(stashOID)
                } catch {
                    throw CodexGitRepositoryError.partialSuccess(
                        "The worktree could not be created and changes remain in stash \(stashOID). Restore them manually before retrying."
                    )
                }
            }
            throw error
        ...
        if let stashOID {
            do {
                ...
                let transferred = try await runAt(
                    ["status", "--porcelain", "-z", "--untracked-files=all"],
                    directory: target,
                    allowTruncation: true
                )
                guard !transferred.stdout.isEmpty else {
                    throw CodexGitRepositoryError.partialSuccess(
                        "The captured changes remain in stash \(stashOID); the new worktree was created without them."
                    )
                }
                _ = try await dropHandoffStash(stashOID)
            } catch {
                // Keep both copies rather than risk data loss. The source is
                // restored and the stash remains available for manual repair.
                _ = try? await run(["stash", "apply", "--index", stashOID])
                throw CodexGitRepositoryError.partialSuccess(
    ...
    private func dropHandoffStash(_ oid: String) async throws -> CodexGitCommandResult {
        let current = try await run(["rev-parse", "refs/stash"])
            .stdout.trimmingCharacters(in: .whitespacesAndNewlines)
        ...
        return try await run(["stash", "drop", "stash@{0}"])
    }
