...
/// Actions for server-owned background terminals. The host supplies these only
/// when a live Codex session can perform the generated protocol methods.
...
public struct CodexFloatingSummaryPanel: View {
    ...
    private let outputs: [CodexOutputSummary]
    ...
    public var body: some View {
        VStack(alignment: .leading, spacing: 18) {
                    ...
                    if backgroundTerminals.terminals.isEmpty {
                        ...
                        ForEach(backgroundTerminals.terminals) { terminal in
                            SummaryBackgroundTerminalRow(
                                terminal: terminal,
                                onTerminate: { backgroundTerminalActions.terminate(terminal.processID) }
                            )
                        }
                    }
                    if let nextCursor = backgroundTerminals.nextCursor {
                        SummaryEmptyRow(title: "More processes available (\(nextCursor))")
                    }
                }
            }
            ...
            SummarySection(title: "Outputs") {
                if outputs.isEmpty {
                    SummaryEmptyRow(title: "No outputs recorded")
                } else {
                    ForEach(outputs.prefix(4)) { output in
                        SummaryOutputRow(output: output)
                    }
                    if outputs.count > 4 {
                        SummaryEmptyRow(title: "\(outputs.count - 4) more outputs in transcript")
                    }
                    Text("Output details are available in the transcript")
        ...
        .onAppear {
            // Listing is a real app-server operation, not a local process probe.
...
private struct SummaryOutputRow: View {
    @Environment(\.codexAgentTheme) private var theme

    let output: CodexOutputSummary
