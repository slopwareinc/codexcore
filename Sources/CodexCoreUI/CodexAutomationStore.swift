...
public struct CodexAutomationFileStore: Sendable {
    ...
    /// Client-only metadata is stored beside each official automation file so
    /// the pinned Codex automation TOML remains unchanged and compatible with
    /// other Codex clients.
    public static let runHistoryFileName = "run-history.json"
    ...
    public func save(_ automation: CodexAutomation) throws {
        ...
        try Self.writeRunHistory(automation.previousRuns, in: automationDirectory)
        cache.store(
            Document(
    ...
    /// The sidecar location is exposed for diagnostics and focused tests; its
    /// presence is not required for loading an automation.
    public func runHistoryURL(for id: String) -> URL {
        directoryURL
            .appendingPathComponent(id, isDirectory: true)
            .appendingPathComponent(Self.runHistoryFileName)
    }
...
private extension CodexAutomationFileStore {
    ...
    static func loadSynchronously(directoryURL: URL, cache: Cache) -> CodexAutomationLoadResult {
        ...
        for directory in directories {
            ...
            do {
                ...
                var automation = document.automation
                do {
                    automation.previousRuns = try readRunHistory(in: directory)
                } catch let error as RunHistoryError {
                    errors.append(CodexAutomationLoadError(
                        url: directory.appendingPathComponent(runHistoryFileName),
                        kind: error.kind,
                ...
                let loadedDocument = Document(
                    ...
                    unknownValues: document.unknownValues,
                    updatedAtMilliseconds: document.updatedAtMilliseconds
                )
                automations.append(automation)
                cache.store(loadedDocument, for: automation.id)
            } catch {
                errors.append(CodexAutomationLoadError(
                    url: file,
                    kind: .parse,
    ...
    enum RunHistoryError: Error {
        case read(String)
        case parse(String)

        var kind: CodexAutomationLoadError.Kind {
            switch self {
            case .read: .runHistoryRead
            case .parse: .runHistoryParse
            }
        }

        var message: String {
            switch self {
            case let .read(message), let .parse(message): message
            }
        }
    }

    static func readRunHistory(in directory: URL) throws -> [CodexAutomationRunRecord] {
        let file = directory.appendingPathComponent(runHistoryFileName)
        guard FileManager.default.fileExists(atPath: file.path) else { return [] }
        let data: Data
        do {
            data = try Data(contentsOf: file)
        } catch {
            throw RunHistoryError.read(error.localizedDescription)
        }
        do {
            let decoder = JSONDecoder()
            decoder.dateDecodingStrategy = .iso8601
            return Array(try decoder.decode([CodexAutomationRunRecord].self, from: data).suffix(CodexAutomation.maxPreviousRunCount))
        } catch {
            throw RunHistoryError.parse(error.localizedDescription)
        }
    }

    static func writeRunHistory(_ runs: [CodexAutomationRunRecord], in directory: URL) throws {
        let file = directory.appendingPathComponent(runHistoryFileName)
        ...
        let data = try encoder.encode(Array(runs.suffix(CodexAutomation.maxPreviousRunCount)))
