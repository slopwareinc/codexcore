...
final class CodexAutomationRouteTests: XCTestCase {
    ...
    func testRunHistoryRecordsPreviousRunsAndRemainsBounded() {
        let now = Date(timeIntervalSince1970: 1_800_000_000)
        var lifecycle = CodexAutomationLifecycle(automations: [CodexAutomation(
            id: "history",
            name: "History",
            prompt: "Record runs",
            createdAt: now
        ...
        for offset in 0..<(CodexAutomation.maxPreviousRunCount + 3) {
            let started = now.addingTimeInterval(Double(offset * 60))
            _ = lifecycle.beginRun(id: "history", now: started)
            lifecycle.finishRun(
                id: "history",
                threadID: "thread-\(offset)",
                error: offset == 2 ? "failed" : nil,
                now: started.addingTimeInterval(10)
            )
        }

        let automation = lifecycle.automations[0]
        XCTAssertEqual(automation.previousRuns.count, CodexAutomation.maxPreviousRunCount)
        XCTAssertEqual(automation.previousRun?.threadID, "thread-\(CodexAutomation.maxPreviousRunCount + 2)")
        XCTAssertEqual(automation.previousRuns.first?.threadID, "thread-3")
        XCTAssertEqual(automation.previousRuns[0].status, .succeeded)
        XCTAssertEqual(automation.lastRunAt, now.addingTimeInterval(Double((CodexAutomation.maxPreviousRunCount + 2) * 60 + 10)))
    }

    func testBulkAutomationActionsPauseAndDeleteDefinitionsWithoutTouchingRunHistory() {
        let now = Date(timeIntervalSince1970: 1_800_000_000)
        var lifecycle = CodexAutomationLifecycle(automations: [
            CodexAutomation(id: "one", name: "One", prompt: "One", createdAt: now),
            CodexAutomation(id: "two", name: "Two", prompt: "Two", status: .disabled, createdAt: now)
        ])
        _ = lifecycle.beginRun(id: "one", now: now)
        lifecycle.finishRun(id: "one", threadID: "thread-1", error: nil, now: now.addingTimeInterval(1))

        let paused = lifecycle.archiveAll()
        XCTAssertEqual(paused.map(\.id), ["one"])
        XCTAssertTrue(lifecycle.automations.allSatisfy { $0.status == .disabled && $0.nextRunAt == nil })
        XCTAssertEqual(lifecycle.automations[0].previousRun?.threadID, "thread-1")

        XCTAssertEqual(lifecycle.deleteAll(), ["one", "two"])
        XCTAssertTrue(lifecycle.automations.isEmpty)
    }

    func testBulkRouteActionsHaveLocalOnlySemantics() {
        XCTAssertEqual(CodexAutomationRouteAction.markAllRead.title, "Mark all runs read")
        XCTAssertEqual(CodexAutomationRouteAction.archiveAll.title, "Pause all automations")
        XCTAssertEqual(CodexAutomationRouteAction.deleteAll.title, "Delete all automations")
        XCTAssertNil(CodexAutomationRouteAction.archiveAll.draftRequest)

        var session = CodexAutomationRouteSession(state: CodexAutomationRouteState(automations: [
            CodexAutomation(id: "one", name: "One", prompt: "One"),
            CodexAutomation(id: "two", name: "Two", prompt: "Two")
        ]))
        _ = session.perform(.archiveAll)
        XCTAssertTrue(session.state.automations.allSatisfy { $0.status == .disabled && $0.nextRunAt == nil })
        _ = session.perform(.deleteAll)
        XCTAssertTrue(session.state.automations.isEmpty)
    }
