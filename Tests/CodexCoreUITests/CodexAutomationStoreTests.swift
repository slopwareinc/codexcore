...
final class CodexAutomationStoreTests: XCTestCase {
    ...
    func testRunHistoryRoundTripsInClientSidecarWithoutChangingTomlSchema() async throws {
        let directory = try makeTemporaryDirectory()
        defer { try? FileManager.default.removeItem(at: directory) }
        ...
        let toml = try String(contentsOf: directory.appendingPathComponent("history/automation.toml"), encoding: .utf8)
        XCTAssertFalse(toml.contains("previousRuns"))
        XCTAssertFalse(toml.contains("previous_runs"))
        XCTAssertTrue(FileManager.default.fileExists(atPath: store.runHistoryURL(for: "history").path))

        let loaded = await store.load()
        XCTAssertTrue(loaded.errors.isEmpty)
        XCTAssertEqual(loaded.automations.first?.previousRuns, automation.previousRuns)

        automation.previousRuns = []
        try store.save(automation)
        XCTAssertFalse(FileManager.default.fileExists(atPath: store.runHistoryURL(for: "history").path))
    }

    func testMalformedRunHistoryDoesNotHideAutomation() async throws {
        let directory = try makeTemporaryDirectory()
        defer { try? FileManager.default.removeItem(at: directory) }
        try writeAutomation(
            id: "history-error",
        ...
        try Data("not json".utf8).write(to: directory.appendingPathComponent("history-error/run-history.json"))

        let result = await CodexAutomationFileStore(directoryURL: directory).load()
        XCTAssertEqual(result.automations.map(\.id), ["history-error"])
        XCTAssertEqual(result.errors.map(\.kind), [.runHistoryParse])
        XCTAssertTrue(result.automations[0].previousRuns.isEmpty)
    }
