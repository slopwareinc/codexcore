...
This is the authoritative user-facing capability matrix for CodexCore `0.10.0` with `codex-cli 0.147.0` or newer. “Visible in the app” does not necessarily mean “wired to production behavior.”
...
| Paginated fork and full `thread/read` turns | Supported | Stable 0.147.0 reads paginated history and forks through the thread store. |
| Paginated rollback | Unsupported | Stable 0.147.0 still rejects `thread/rollback` for paginated threads. |
