...
The canonical projection keeps the opening prompt in `CodexTurnV2.userMessage` and appends every echoed or optimistic `turn/steer` input to `steeredMessages`. Custom transcript renderers should show those values as distinct user bubbles after the active work block and before the final answer. Reconcile each optimistic steer by `clientID`; never replace the opening prompt or render the server echo twice.
