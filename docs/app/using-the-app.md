...
- generic `@` skill mentions, rendered as removable composer tags and submitted
...
Typing `@` searches both files and installed skills. Choosing a skill attaches
it as a tag; the composer contains no skill-specific special case. The bundled
Visualize skill uses this same path and its result renders inline in the
conversation.
